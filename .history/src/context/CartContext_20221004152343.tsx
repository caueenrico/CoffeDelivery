...
interface CartContextType{
  ...
  changeCartItemQuantity: (cartItemId: number, quantity: number) => void
}
...
export function CartContextProvider({children}: CartProviderProps){
  ...
  function changeCartItemQuantity(cartItemId: number,type: 'increment' | 'decrement'){
    const newCart = produce(cartItenms, (draft) => {
      const coffeExistsInCart = draft.findIndex(item => cartItem.id === cartItemId)
