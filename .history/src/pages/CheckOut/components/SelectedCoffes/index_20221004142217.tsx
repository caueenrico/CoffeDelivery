...
export function SelectedCoffes({ coffee }: CoffeeProps) {
  ...
  function quantityCartItens() {
    const quantidadeNoCarrinho = cartItens.filter((item) => item.id === coffee.id);
    return quantidadeNoCarrinho.length;
  }

  const [quantity, setQuantity] = useState(cartItens[0].quantity);
